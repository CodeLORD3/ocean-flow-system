...
export function useShopOrders(storeId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("shop_orders")
        .select("*, desired_delivery_date, packer_name, invoice_status, stores(name), shop_order_lines(*, products(name, unit, category, hs_code, weight_per_piece, wholesale_price))")
