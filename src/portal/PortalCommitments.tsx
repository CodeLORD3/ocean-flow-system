        ...
        <table className="w-full text-[11px]">
          ...
          <tbody>
            {pledges.map((p: any) => {
              ...
              return (
                <tr key={p.id} className="border-b border-[#1a2035]/50 hover:bg-[#0066ff]/5 transition-colors">
                  <td className="p-2 pl-3 text-[#c8d6e5] font-medium">{offer?.title || "—"}</td>
                  <td className="p-2 text-right text-[#c8d6e5]">{Number(p.amount).toLocaleString()} kr</td>
                  <td className="p-2 text-right text-green-400 font-bold">{rate.toFixed(1)}%</td>
                  <td className="p-2 text-right text-[#c8d6e5] font-bold">{expectedReturn.toLocaleString(undefined, { maximumFractionDigits: 0 })} kr</td>
                  ...
                  <td className="p-2 pr-3 text-center">
                    <span className={`inline-block px-2 py-0.5 text-[9px] tracking-wider border ${
                      ...
                      "text-[#5a6a7a] border-[#1a2035] bg-[#1a2035]"
                    }`}>
