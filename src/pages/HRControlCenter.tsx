    ...
    <IndustryFrame className="ind-page space-y-5 p-4 md:p-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="coverage" className="mt-5 space-y-4">
          ...
          <div className="overflow-x-auto"><table className="min-w-[780px] w-full"><thead><tr><th>Enhet</th>{week.map((day, index) => <th key={day}>{DAY_NAMES[index]}<br /><span className="ind-muted font-normal">{day.slice(5)}</span></th>)}<th>Veckans pass</th></tr></thead><tbody>{coverage.map(({ store, days, hours }) => <tr key={store.id}><td className="font-medium">{store.name}</td>{days.map(({ day, scheduled, target }) => { const isShort = target > 0 && scheduled < target; return <td key={day} className={isShort ? "ind-coverage-short" : ""}><span className={isShort ? "ind-status--alert ind-mono font-semibold" : "ind-mono"}>{scheduled}/{target}</span>{isShort && <span className="block text-[10px] ind-status--alert">{target - scheduled} saknas</span>}</td>; })}<td className="ind-mono">{hours}</td></tr>)}</tbody></table></div>
        ...
        <DialogContent className="ind max-w-lg">
          ...
          {selectedRequest && (
            <div className="space-y-4">
              ...
              <div><SectionLabel>Skäl vid avslag</SectionLabel><Textarea value={rejectReason} onChange={(event) => setRejectReason(event.target.value)} placeholder="Krävs om ansökan avslås" />{selectedRequest && !rejectReason.trim() && <p className="ind-muted mt-1 text-xs">Ett skäl krävs innan ansökan kan avslås.</p>}</div>
            </div>
          )}
          <DialogFooter>
