...
export default function TradeOfferDetail({ offer, pledges, onBack, onStatusChange }: TradeOfferDetailProps) {
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      ...
      {/* Return (Investor View) - prominent */}
      ...
      {pledges.length > 0 && (
        <Card>
          <CardContent className="p-3">
            <h3 className="text-[11px] font-bold text-primary tracking-wider mb-2">PLEDGES ({pledges.length})</h3>
            <div className="space-y-1">
              {pledges.map(p => (
                <div key={p.id} className="flex justify-between text-[10px] py-1 border-b border-border/50 last:border-0">
                  <span className="text-muted-foreground">{new Date(p.created_at).toLocaleDateString("sv-SE")}</span>
                  <span className="font-medium">{Number(p.amount).toLocaleString()} kr</span>
