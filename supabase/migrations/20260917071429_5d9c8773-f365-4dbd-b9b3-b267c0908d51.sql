...
BEGIN
  ...
  IF _old IS NOT NULL AND _old > 0 AND _ids IS NOT NULL THEN
    ...
    WHERE l.id = ANY(_ids)
      ...
      AND COALESCE(l.price_status, 'preliminar') <> 'bekraftad';
  END IF;
  ...
  INSERT INTO public.activity_logs (portal, action_type, entity_type, entity_id, description, performed_by, details)
  VALUES (
    'wholesale',
    'finalize_lot_price',
    'lot',
