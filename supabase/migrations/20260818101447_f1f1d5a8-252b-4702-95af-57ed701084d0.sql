-- 1. Reservkedjan: standardkostnadsgruppen är ett nummer (short_identifier)
  ...
  IF v_store IS NULL AND v_default_cg IS NOT NULL THEN
    SELECT store_id INTO v_store FROM public.pk_costgroups
     WHERE connection_id = NEW.connection_id
       AND (short_identifier::text = v_default_cg OR url = v_default_cg OR name = v_default_cg)
     LIMIT 1;
  END IF;
...
-- 2. Samma matchning i vyn "På plats nu"
 ...
 SELECT j.connection_id,
   ...
   FROM joined j
     ...
     LEFT JOIN pk_costgroups dcg ON dcg.connection_id = j.connection_id
       AND (dcg.short_identifier::text = ps.default_cost_group OR dcg.url = ps.default_cost_group
            OR dcg.name = ps.default_cost_group)
...
-- 3. Testfall: ändra och avboka en Personalkollen-stämpling, verifiera speglingen, återställ
DO $test$
DECLARE
  r public.pk_logged_times;
  m1 timestamptz; m2 timestamptz; n_after_cancel integer; m3 timestamptz;
  txt text;
BEGIN
  SELECT l.* INTO r FROM public.pk_logged_times l
    JOIN public.staff_shifts ss ON ss.pk_logged_time_id = l.id
   WHERE NOT l.is_canceled
   ORDER BY l.start DESC LIMIT 1;

  IF r.id IS NULL THEN RETURN; END IF;

  SELECT clocked_in_at INTO m1 FROM public.staff_shifts WHERE pk_logged_time_id = r.id;

  -- (a) tid ändras i Personalkollen
  UPDATE public.pk_logged_times SET start = r.start - interval '7 minutes', real_start = NULL WHERE id = r.id;
  SELECT clocked_in_at INTO m2 FROM public.staff_shifts WHERE pk_logged_time_id = r.id;

  -- (b) stämplingen avbokas i Personalkollen
  UPDATE public.pk_logged_times SET is_canceled = true WHERE id = r.id;
  SELECT count(*) INTO n_after_cancel FROM public.staff_shifts WHERE pk_logged_time_id = r.id;

  -- (c) återställ originalet
  UPDATE public.pk_logged_times
     SET is_canceled = r.is_canceled, start = r.start, real_start = r.real_start
   WHERE id = r.id;
  SELECT clocked_in_at INTO m3 FROM public.staff_shifts WHERE pk_logged_time_id = r.id;

  txt := format(
    'testfall spegling: url=%s | fore=%s | efter tidsandring=%s (vantat %s) | rader efter avbokning=%s (vantat 0) | efter aterstallning=%s (vantat %s)',
    r.url, m1, m2, r.start - interval '7 minutes', n_after_cancel, m3, coalesce(r.real_start, r.start));

  INSERT INTO public.pk_sync_log (connection_id, resource, pages, upserts, status, error)
  VALUES (r.connection_id, 'mirror-test', 0, 0,
          CASE WHEN m2 = r.start - interval '7 minutes' AND n_after_cancel = 0
                    AND m3 = coalesce(r.real_start, r.start) THEN 'ok' ELSE 'error' END,
          txt);
END
$test$;