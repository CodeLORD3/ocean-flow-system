...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  att AS (
    SELECT a.employee_id,
           sum(coalesce(a.approved_minutes, 0))::numeric AS approved_minutes,
  ...
  ob AS (
    SELECT emp.employee_id,
           ...
           sum(d.regular_minutes)::numeric AS regular
  ...
  SELECT emp.employee_id, emp.employment_id, emp.full_name, emp.employment_number,
         ...
         b.f, b.t,
         round(coalesce(att.approved_minutes, ob.regular, 0) / 60.0, 2),
         round(coalesce(ob.ob50, 0) / 60.0, 2),
         round(coalesce(ob.ob70, 0) / 60.0, 2),
         round(coalesce(ob.ob100, 0) / 60.0, 2),
...
REVOKE EXECUTE ON FUNCTION public.payroll_basis_period(text, text) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.payroll_basis_period(text, text) TO authenticated, service_role;